...
export class ConfidenceScoringEngine {
  ...
  calculateConfidenceScore(
    ...
    try {
      ...
      // Handle DOM errors gracefully and continue scoring process

      warnings.push('Could not validate selector uniqueness due to DOM error');
      // Add a minimal uniqueness penalty when validation fails
      score -= 10;
      factors.push({
        factor: 'uniqueness_error',
        impact: 'negative',
        weight: 0.10,
        description: 'Unable to validate selector uniqueness'
      });
    }
